...
/// （web 前端登录页 + httpOnly session cookie），不经过 Rust。同一 app 内的 webview
/// 共享持久化 cookie 存储：用户在首页登录后，稍后打开「同步盘」窗口触发 `open_login` 时，
/// 登录 webview 会因 cookie 已在而立即命中 `/desktop` 自动换 device JWT，无需重新登录。
...
///
/// httpOnly session cookie 页面 JS 读不到，但 native 层可经 `cookies_for_url` 读回
/// （wry WKWebView 后端保留 httpOnly 标记）。因此流程全在 Rust 侧闭环：
/// 建 webview → `on_navigation` 命中 `/desktop` → 读 session cookie → 换 device JWT →
/// 存 keychain → emit "authed" → 关窗。比依赖远程页面 IPC 回传更稳。
#[tauri::command]
pub(super) async fn open_login(app: tauri::AppHandle, core: State<'_, Arc<AppCore>>) -> Result<(), String> {
    ...
    let win = tauri::WebviewWindowBuilder::new(
        &app,
        LOGIN_WINDOW,
        tauri::WebviewUrl::External(login_url().parse().map_err(|e| format!("bad login url: {e}"))?),
    )
    .title("登录来福同步盘")
    .inner_size(480.0, 640.0)
    .on_navigation(move |url| {
        // gateway 登录成功 302 到 /desktop；命中即触发换 token。放行导航（返回 true）。
        if url.path().contains("/desktop")
...
async fn complete_login(app: &tauri::AppHandle, core: &Arc<AppCore>) -> Result<(), String> {
    ...
    let cookies = win.cookies_for_url(gw_url).map_err(|e| e.to_string())?;

    let name = session_cookie_name();
    let sid = cookies
        .into_iter()
        .find(|c| c.name() == name)
        .ok_or_else(|| format!("session cookie `{name}` not found after login"))?;
    let cookie_header = format!("{}={}", sid.name(), sid.value());
...
pub(super) async fn complete_desktop_oauth(app: &tauri::AppHandle, core: &Arc<AppCore>, code: &str) -> Result<(), String> {
    let tok = core.gateway.device_token_exchange(code).await.map_err(|e| e.to_string())?;
    ...
    *core.auth.lock().await = AuthState::authed_from(tok);
    ...
    if let Some(home) = app.get_webview_window(HOME_WINDOW) {
        let sc = core.gateway.session_code(&jwt).await.map_err(|e| e.to_string())?;
...
#[tauri::command]
pub(super) async fn open_oauth_in_browser(app: tauri::AppHandle, core: State<'_, Arc<AppCore>>, provider: String) -> Result<(), String> {
    use tauri_plugin_opener::OpenerExt;
    let channel = crate::channel::name();
    let url = format!("{}/api/auth/{provider}/start?client=desktop&channel={channel}", core.gateway.base_url());
