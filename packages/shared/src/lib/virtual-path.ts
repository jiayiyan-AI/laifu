 ...
 *
 * 长度以 **JS UTF-16 code unit** 计（即 `string.length`），不是 UTF-8 字节数。
 * 一个 4 字节 emoji（如 🎉）占 2 个 code unit。Azure Blob 路径上限是 1024 UTF-8 字节，
 * 本校验比那略宽松；如果未来需要严格按字节限制，把长度计算换成
 * `new TextEncoder().encode(seg).length`。
 */
export function validateVirtualPath(path: string): ValidationResult {
  ...
  for (const seg of segments) {
    // trailing '/' was already caught above; this fires only for internal '//'
    if (seg.length === 0) {
