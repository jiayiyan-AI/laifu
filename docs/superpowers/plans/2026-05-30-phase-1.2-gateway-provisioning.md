...
**Goal:** Stand up monorepo 骨架、Gateway Node 进程、本地 Supabase、本地 stub provisioner，所有功能在本地端到端验证。**完成标志**：本地 `pnpm dev` + 本地 Supabase 跑通：POST `/api/purchase` 立即返 200，本地 stub provisioner 走完 6 步（约 5s）后 status=ready / container_url=`http://localhost:8080`；GET `/api/status` 反映进度；重启 Gateway 启动恢复正常。

**修订说明（2026-05-30）**：原 Task 13/14 涉及 Azure 真实部署的部分推迟到 Phase 1.5 plan。本 Phase 仅本地验证。Bicep 模板（Task 12）和 Azure SDK wrappers（Task 7）仍然完成，但**不在本 Phase 调用 Azure**。
...
## Task 13: 本地 stub provisioner（替代原 Azure 部署）

新增 local provisioner，让本地无需 Azure 就能模拟 6 步 provisioning 流程，最终把 container_url 写成 `http://localhost:8080`。Env var `PROVISIONER=local|azure` 切换。

**Files:**
- Create: `apps/gateway/src/provisioning/local.ts`
- Create: `apps/gateway/test/provisioning/local.test.ts`
- Modify: `apps/gateway/src/index.ts`
- Modify: `apps/gateway/.env.example`
- Modify: `apps/gateway/src/config.ts`

- [ ] **Step 13.1: 改 `apps/gateway/.env.example`** — 加 `PROVISIONER` 和 `LOCAL_CONTAINER_URL`
...
# Provisioner mode: 'local' (dev) | 'azure' (prod)
PROVISIONER=local
LOCAL_CONTAINER_URL=http://localhost:8080

# Azure (仅 PROVISIONER=azure 时需要)
AZURE_SUBSCRIPTION_ID=
AZURE_RESOURCE_GROUP=rg-lingxi-dev
AZURE_LOCATION=eastasia
AZURE_CONTAINER_APPS_ENV=cae-lingxi-dev
...
- [ ] **Step 13.2: 改 `apps/gateway/src/config.ts`** — 加 `provisioner` 和 `localContainerUrl` 两个字段
...
export const config = {
  ...
  // ... 现有 supabase/azure 子字段 ...
};

// validateConfig 改为按 provisioner 校验
export const validateConfig = () => {
  required('SUPABASE_URL');
  required('SUPABASE_SERVICE_ROLE_KEY');
...
- [ ] **Step 13.3: 写 failing test for local provisioner**

`apps/gateway/test/provisioning/local.test.ts`:
...
预期：`Cannot find module '../../src/provisioning/local.js'`。

- [ ] **Step 13.5: 实现 `apps/gateway/src/provisioning/local.ts`**
...
预期：15 tests pass（前 14 + 新 1）。

- [ ] **Step 13.7: 改 `apps/gateway/src/index.ts`** — defaultProvisioner 根据 config 切换

替换 createApp 里 `defaultProvisioner` 定义为：
...
`recoverProvisioning` 只对 azure 模式有意义（local 模式没有"卡在云上"的状态，进程死了就死了）。改 index.ts 的启动块：
...
预期：15 tests pass，lint clean。

- [ ] **Step 13.10: Commit**

```bash
git add apps/gateway/
git commit -m "feat(gateway): add local stub provisioner for dev-time validation"
```

---

## Task 14: 用户手工 — 本地 Supabase 起步

非 TDD，用户在自己 Mac 上操作。无法委托给 subagent。

- [ ] **Step 14.1: 安装 Supabase CLI**
...
- [ ] **Step 14.2: 在 `infra/supabase/` 下 init**

```bash
cd /Users/yanjiayi/workspace/laifu/infra/supabase
supabase init
```

会生成 `config.toml`。如果已经存在 `migrations/` 目录（Task 4 已经放进去），CLI 会保留。

- [ ] **Step 14.3: 起本地 Supabase**
...
第一次会拉几个 Docker image（Postgres + GoTrue + PostgREST 等），耗时 1-2 分钟。完成后输出：

```
API URL:        http://127.0.0.1:54321
DB URL:         postgresql://postgres:postgres@127.0.0.1:54322/postgres
Studio URL:     http://127.0.0.1:54323
service_role key: eyJxxx...（很长）
anon key:         eyJyyy...
```

**记下 `API URL` 和 `service_role key`**。
...
会把 `migrations/0001_init.sql` 应用到本地 DB。预期最后输出 `Finished supabase db reset`。

- [ ] **Step 14.5: 验证表存在**

打开 `http://127.0.0.1:54323`（Studio），左侧 Tables 应看到 5 张表。
...
- [ ] **Step 14.6: 填 `apps/gateway/.env.local`**
...
# 编辑 .env.local，填：
#   SUPABASE_URL=http://localhost:54321
#   SUPABASE_SERVICE_ROLE_KEY=<Step 14.3 拿的 service_role key>
#   PROVISIONER=local
#   LOCAL_CONTAINER_URL=http://localhost:8080
# （Azure 那几行留空，PROVISIONER=local 时不校验）
```

- [ ] **Step 14.7: Gateway 启动冒烟**
...
（PROVISIONER=local 时不会跑 recoverProvisioning。）
...
非 TDD，手动验证 Phase 1.2 验收标准。

- [ ] **Step 15.1: 启 Gateway 保持运行**
...
预期 5 秒内看到所有 6 步：
```
{"status":"provisioning","provisioning_step":"正在创建账户与订单","progress_pct":5,...}
{"status":"provisioning","provisioning_step":"正在生成数字助理实例","progress_pct":20,...}
{"status":"provisioning","provisioning_step":"为助理分配 DID 与 Agent 运行时","progress_pct":40,...}
{"status":"provisioning","provisioning_step":"初始化默认能力","progress_pct":70,...}
{"status":"provisioning","provisioning_step":"装载基础知识库","progress_pct":90,...}
...
打开 Studio `http://127.0.0.1:54323` → Tables → `container_mapping`：
- ✅ 应有一行
- ✅ `status='ready'`
- ✅ `container_url='http://localhost:8080'`
- ✅ `container_name='hermes-11111111'`（短哈希）

- [ ] **Step 15.5: 测启动恢复 no-op（local 模式跳过）**

`Ctrl+C` 停 Gateway。重新启：
```bash
pnpm exec tsx -r dotenv/config src/index.ts
```

预期 **不应**看到 `recovering stuck provisioning rows`（local 模式跳过）。看到 `loading cache...` 和 `listening on :3000`。
...
预期：不会重复 provisioning，返回当前 status (`ready`)。

- [ ] **Step 15.7: 清理测试残留**

```bash
PGPASSWORD=postgres psql -h 127.0.0.1 -p 54322 -U postgres -d postgres \
  -c "delete from container_mapping where user_id='11111111-2222-3333-4444-555555555555';"
```

- [ ] **Step 15.8: 记录验证结果**

新建 `docs/superpowers/plans/2026-05-30-phase-1.2-verification.md`:
...
- 验证时间：YYYY-MM-DD
- DoD 项：
  - [x] `pnpm -r test` 全 pass（15 tests）
  - [x] `pnpm -r lint` 全无报错
  - [x] POST /api/purchase < 1s 返 provisioning
  - [x] GET /api/status 5 秒内看到 6 步进度推进到 ready
  - [x] container_url=http://localhost:8080
  - [x] container_mapping 行在本地 DB 正确
  - [x] 重复购买幂等

Phase 1.2 本地验收通过。Hermes Image / Web 前端 / 微信端 / Azure 部署
分别由 Phase 1.1 / 1.3 / 1.4 / 1.5 完成。
```

```bash
git add docs/superpowers/plans/2026-05-30-phase-1.2-verification.md
git commit -m "docs: Phase 1.2 local verification record"
```

---

## DoD（Phase 1.2 本地验收标准）

全部以下 ✅，Phase 1.2 视为完成：

1. ✅ `pnpm -r test` 全 pass（包含 local provisioner 测试，共 15 tests）
2. ✅ `pnpm -r lint` 全无报错
3. ✅ 本地 Supabase 起步，5 张表创建好
4. ✅ POST /api/purchase < 1 秒返回 provisioning
5. ✅ GET /api/status 5 秒内看到 6 步进度推进到 ready
6. ✅ `container_url='http://localhost:8080'`（stub provisioner 写入）
7. ✅ 本地 DB 中 container_mapping 行正确

**显式不包含**：Azure 真实部署、真实 Container App 创建、云上 Supabase。这些移到 Phase 1.5 plan。
