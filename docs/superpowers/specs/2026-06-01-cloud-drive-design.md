...
| `virtual_path` 校验 + 云盘 contracts | `packages/shared/src/lib/virtual-path.ts` + `packages/shared/src/contracts.ts` | 新增 / 扩展 |
| `user_entitlements` + `container_observed_state` 表 + RLS | Supabase | 新增 |
| `users.token_version` 列 | Supabase (ALTER TABLE) | 新增 |
| Azure Blob container `laifu-cloud` | Azure (storage account 启用 HNS / ADLS Gen2) | 新增 |
...
- Storage account：`laifuprod`（prod）/ `laifu-dev`（dev）
  - **必须启用 Hierarchical Namespace（即 ADLS Gen2）** — 这是 directory-scoped SAS（`sr=d` + `sdd`）的前提。普通扁平 Blob storage 无法签发限定 prefix 写权限的 SAS，会退化成 container SAS（racwl 覆盖整个 container，跨用户隔离失效）。
- Blob container：`laifu-cloud`
- Blob 路径：`<user_id>/<virtual_path>`
- `user_id`：Supabase users 表的 UUID 主键，作为一级"目录"（HNS 真目录，非纯字符串前缀）
...
| key | 类型 | 编码 | 必/选 | 说明 |
|---|---|---|---|---|
| `title` | string | base64-utf8 | 必 | 用户可见的标题。CLI 缺省时 tool 用 virtual_path 的 basename 兜底。中文/emoji 上传时 base64 编码；**gateway list 时统一解码为 UTF-8**，客户端（web / 其他）拿到的是已解码字符串（见 §五 `/api/cloud/list` 契约）。 |
...
校验工具放 **`packages/shared/src/lib/virtual-path.ts`**（laifu monorepo 已有 `packages/shared`，是 gateway / web 都能 import 的共享层）。gateway 不能从 `apps/web/` import，web 也不能从 `apps/gateway/` import —— 共享代码必须在 `packages/shared`。

`packages/shared/src/contracts.ts` 同时扩展加云盘相关 types（`CloudFileItem`、`CloudFolderItem`、`SasResponse`、`StatusResponse` 扩展字段等），gateway 和 web 都引用同一份契约。
...
CREATE TABLE user_entitlements (
  ...
  disabled_at TIMESTAMPTZ,                -- NULL = 当前启用；NOT NULL = 已停用
  metadata    JSONB,                       -- 留扩展位（套餐版本号等）
  PRIMARY KEY (user_id, feature)
);

-- "active" 的唯一判定标准: disabled_at IS NULL
-- 加索引便于活跃查询
CREATE INDEX user_entitlements_active ON user_entitlements (user_id, feature)
...
**active 判定**：所有"是否开通 X"的查询必须用 `WHERE user_id = $1 AND feature = $2 AND disabled_at IS NULL`，**不要**只看行是否存在。

**enable 语义**（无论之前 row 存在与否、是否曾被 disable）：

```sql
INSERT INTO user_entitlements (user_id, feature)
VALUES ($1, $2)
...
SET disabled_at = NOW()
WHERE user_id = $1 AND feature = $2 AND disabled_at IS NULL;
```

不删 row，blob 数据也保留；重新 enable 时无缝恢复。
...
2. gateway:
   a. UPSERT user_entitlements (清空 disabled_at, 更新 enabled_at)
   b. UPDATE users SET token_version = token_version + 1
   c. 生成新 JWT (含新 token_version) 传给 provisioning
        ↓
3. gateway 调 Azure SDK 重启该用户的 Container App (restartRevision)，注入新 LAIFU_USER_TOKEN
        ↓
4. 容器启动，entrypoint.sh:
     a. 从 env 拿 LAIFU_USER_TOKEN (JWT)
     b. 调 gateway /api/me/entitlements 拉当前 desired entitlements
        → 返回 ["cloud"]
     c. 按 entitlements 软链对应 skill 到 hermes skill 目录：
        ln -snf /opt/hermes-skills/cloud_publish ~/.hermes/skills/cloud_publish
     d. ★ 软链全部完成后调 gateway POST /api/me/observed-entitlements
        body: { observed: ["cloud"], token_version: <从 JWT 解出> }
        gateway 写到 container_observed_state 表（或 ContainerMappingCache）
        ↓
5. hermes 启动，扫 ~/.hermes/skills/，加载 cloud_publish
        ↓
6. agent 现在能看到 cloud-publish CLI 这个工具
        ↓
7. web 端：等待 Modal 轮询 /api/status，等到
       desired_entitlements ⊇ {'cloud'}
       AND observed_entitlements ⊇ {'cloud'}
       AND containerStatus === 'running'
   ★ 关键：要等 step 4d 上报，才算"真正生效"，不是 DB 写完就 true
   → 关闭 Modal，Dock 上 📁 Files 图标淡入，自动打开 Files App
```

停用流程对称：bump token_version + UPDATE `disabled_at` → 重启 → entrypoint 软链不再创建 → 上报 observed 不含 cloud。**不删除 Blob 数据**，下次重新启用看到原状。

### 容器侧状态反馈表

新增 Supabase 表（或挂在现有 ContainerMappingCache 里）：

```sql
CREATE TABLE container_observed_state (
  user_id              UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  observed_entitlements TEXT[] NOT NULL DEFAULT '{}',
  observed_token_version INT NOT NULL DEFAULT 0,
  reported_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
```

每次容器 entrypoint 上报后 UPSERT 这张表。`/api/status` 读它合并到响应里。
...
payload: {
  "user_id": "<uuid>",
  "token_version": <int>,    ← 与 users.token_version 比对，不等则拒
  "iat": <epoch>,
  "exp": <iat + 90 days>     ← 90 天过期
}
...
Hermes agent 在容器内能跑 shell —— 任何 `echo $LAIFU_USER_TOKEN` / `env` 的 prompt injection 都能把 token 抽出去。一旦泄露：
- 无限期 token：泄露 = 平台事故，必须全局换 GATEWAY_SECRET
- 90 天 + token_version：撤销只需 bump 这一个用户的 version，下次容器重启即生效

#### 验签时检查项

```typescript
function verifyContainerToken(jwt) {
  const payload = jwt.verify(token, GATEWAY_SECRET, { algorithms: ['HS256'] });
  // 1. exp 检查由 jwt.verify 自动做
  // 2. token_version 必须匹配当前 users 表
  const current = await db.users.findOne({ id: payload.user_id });
  if (payload.token_version !== current.token_version) {
...
#### 续签：`POST /api/auth/refresh-token`

容器启动时 entrypoint 检查 token：

```bash
# pseudo
expires_at = parse_exp_from_jwt($LAIFU_USER_TOKEN)
if expires_at - now < 7 days:
  new_token = curl -X POST gateway/api/auth/refresh-token \
                   -H "Authorization: Bearer $LAIFU_USER_TOKEN"
  # gateway 校验旧 token（允许 grace 期：exp 在过去 7 天内也接受）
  # 然后用 users.token_version 当前值签新 token (90 天)
  # 容器更新自己 env 文件 + reload
```

Grace 期处理：
- 容器睡 < 90 天：醒来 token 仍 valid，正常用；若 <7 天到期，启动时自动续签
- 容器睡 90 ~ 97 天：token 已过期 7 天内，refresh-token 仍然接受（grace），续签成功
- 容器睡 > 97 天：refresh-token 拒，容器需 web 端用户重新触发 enable（实际上几乎不会发生 —— 一个用户 3 个月不用助理算流失了）

#### 应急轮换路径（运维 runbook，`docs/runbooks/gateway-secret-rotation.md`）
...
GET    /api/status                        现有接口扩展 desired_entitlements + observed_entitlements + containerStatus
GET    /api/me/entitlements               容器 entrypoint 拉自身 desired 权益（JWT 鉴权）
POST   /api/me/observed-entitlements      容器 entrypoint 软链完成后回报实际加载的 skill（JWT 鉴权）
POST   /api/auth/refresh-token            容器启动时若 token 临近过期 → 续签（JWT 鉴权 + grace 期允许） 
...
逻辑:
  1. UPSERT (见 §三的 SQL):
       INSERT ... ON CONFLICT (user_id, feature) DO UPDATE
         SET disabled_at = NULL, enabled_at = NOW();
       返回是否发生了状态变更 (从 disabled/不存在 → active)
  2. bump users.token_version (见 §四 JWT 设计) — 即刻让旧 LAIFU_USER_TOKEN 失效
  3. 状态变更 = true 时（首启用 或 重新启用）：
       a. 生成新 JWT (含新 token_version)，传给 provisioning 注入容器 env
       b. 调 Azure SDK restartRevision 重启该用户的 Container App
       c. 不等 health (不阻塞响应)；前端用 observed_entitlements 反馈环验证
  4. 返回 { ok: true, entitlements: [...] }

幂等: 已 active 时直接返回 entitlements；不 bump token_version，不重启
```

### `POST /api/entitlements/cloud/disable`

```
auth: requireSession

逻辑:
  1. UPDATE ... SET disabled_at = NOW() WHERE user_id=$1 AND feature='cloud' AND disabled_at IS NULL
  2. bump users.token_version — 旧 token 即刻失效
  3. 重启容器（带新 token + 不含 'cloud' 的 entitlement 列表）
  4. 返回 { ok: true, entitlements: [...] }
...
逻辑:
  ...
  2. 用 DK 签 directory-scoped User Delegation SAS:
     - signedResource         sr=d            ← directory scope，关键
     - signedDirectoryDepth   sdd=1           ← user_id 占 1 层目录
     - signedVersion          sv=2020-02-10 或更高（sdd 是 2020-02-10 引入的）
     - canonicalizedResource  /blob/<account>/laifu-cloud/<user_id>
     - signedPermissions      racwl           (read / add / create / write / list)
     - signedStart            now
     - signedExpiry           now + 15min
     - signedProtocol         spr=https
     - signedKey              (UDK)
  3. SAS 严格只能操作 <user_id>/ 子树下的 blob，跨前缀的请求 Azure 服务端直接 403。
  4. 返回:
     {
       blob_endpoint: "https://laifuprod.blob.core.windows.net",
       container: "laifu-cloud",
       prefix: "<user_id>/",
       sas_token: "sv=2020-02-10&sr=d&sdd=1&sp=racwl&...&sig=...",
       expires_at: "2026-06-01T12:15:00Z"
     }

注：directory-scoped SAS 要求 storage account 启用 Hierarchical Namespace (ADLS Gen2)。
若未启用，签出来的只能是 container SAS（sr=c），权限会覆盖整个 container —— 多租户隔离失效。
P0 阶段创建 storage account 时必须勾选 HNS。
```
...
逻辑:
  ...
  3. 拆分结果（gateway 在此处统一解码 base64-utf8 metadata，前端拿到的就是 UTF-8 字符串）:
     folders: 子目录 (去掉 <user_id>/ 前缀)
     files: 每个 blob 的:
       - virtual_path (去掉 <user_id>/ 前缀)
       - size, last_modified, content_type
       - metadata: {
           title:       <decoded UTF-8 string>,
           session_id:  <ASCII string | null>,
           published_at: <ISO-8601 string>,
           tool_version: <ASCII string>,
           description: <decoded UTF-8 string | null>,
           tags:        <string[] | null>     // 已 split(',')
         }
  4. 返回 { folders, files }
```

**契约**：`packages/shared/src/contracts.ts` 里 `CloudFileItem.metadata` 字段类型明确为已解码 UTF-8 string。**前端不再做 base64 解码**，直接用。
gateway 是 base64 解码的唯一一处，保证客户端代码（web / hermes skill）实现一致性。
```
...
参数:
  path        必填，相对 <user_id>/ 的虚拟路径
  dispose     可选，'inline' (默认，浏览器看 Content-Type 自决) | 'attachment' (强制下载)

逻辑:
  1. 校验 path 合法 (不含 ..、不允许跨用户前缀；用 packages/shared 的 virtual-path 校验)
  2. full_path = `${user_id}/${path}`
  3. blob HEAD 一次确认存在 (不存在 → 404)，同时拿到 metadata.title 用于文件名
  4. 签 blob-scoped read-only SAS (sr=b, sp=r, TTL 5 分钟):
       - 如果 dispose === 'attachment':
           SAS 加 rscd 参数: `attachment; filename*=UTF-8''<RFC5987 encoded title>`
           SAS 加 rsct 参数: 强制 application/octet-stream 也 OK，但通常不必
       - 如果 dispose === 'inline' (默认):
           不加 rscd，浏览器看 blob 自带 Content-Type 决定渲染方式
           PDF / 图片 → 直接 inline 显示
           其他 (md/txt/zip) → 浏览器默认行为 (大多是下载)
  5. 302 redirect 到 `${blob_endpoint}/${container}/${full_path}?${sas}`

浏览器直连 Blob，不流过 gateway。

中文文件名 RFC 5987 编码示例:
  title (decoded UTF-8): "销售报告.pdf"
  rscd: attachment; filename*=UTF-8''%E9%94%80%E5%94%AE%E6%8A%A5%E5%91%8A.pdf
  encodeURIComponent 即可。
```
...
| 触发场景 | 调用 | 浏览器行为 |
|---|---|---|
| 双击 PDF / 图片 (预览) | `/api/cloud/download?path=...` (默认 inline) | iframe/img src 设为此 URL，跟 302 到 Blob，inline 渲染 |
| 详情浮层 [下载] 按钮 | `/api/cloud/download?path=...&dispose=attachment` | window.open，浏览器收到 rscd 强制下载，保留原文件名（含中文） |
| 双击 .md/.txt/.csv | `/api/cloud/download?path=...&dispose=attachment` | 直接下载（MVP 不嵌入预览） |
...
PreviewModal.tsx (~40 行)
  ...
  - iframe / img src 用 /api/cloud/download?path=...  (默认 dispose=inline)
    浏览器跟着 302 到 Blob，按 Content-Type 自动 inline 渲染

预览分发 (~20 行)
  - .pdf → <iframe src={`/api/cloud/download?path=${encodeURIComponent(path)}`}>
  - .png .jpg .jpeg .gif .webp → <img src={同上}>
  - 其他 → 不弹 modal，window.open('...&dispose=attachment') 走下载

集成到 FileList (~10 行)
  - 双击文件按扩展名分流（预览 or 下载）
  - 详情浮层"下载"按钮 → 始终 attachment
```

**publish tool 必须**：上传时设对 `content-type`（Python `mimetypes.guess_type` 推断），否则浏览器默认按 `application/octet-stream` 处理会丢失 inline 行为。这是预览能正常工作的前提。
...
async function navigate(path: string) {
  setLoading(true);
  // gateway 已经把 metadata.title/description/tags 解码成 UTF-8 string
  // 前端不再做 base64 解码（解码责任唯一在 gateway，见 §五 /api/cloud/list 契约）
...
逻辑:
  step 1: POST /api/entitlements/cloud/enable → 200 表示 DB 已写 + token bump + restart 触发
          ("正在记录权益...")
  step 2: 轮询 GET /api/status 每 2s 一次，等到：
            containerStatus === 'running'                          // ACA 健康
            AND desired_entitlements.includes('cloud')             // gateway 写表了
            AND observed_entitlements.includes('cloud')            // ★ 容器 entrypoint 已经软链完毕并回报
          最长 30s
          ("助理重启中...")
...
`/api/status` 响应里同时返回 `desired_entitlements` 和 `observed_entitlements`。前者来自 user_entitlements 表，后者来自 container_observed_state 表。
**前端必须等到 observed ⊇ desired** 才算启用完成；否则只是 DB 写了但容器还没生效。
...
| 模块 | 关键用例 |
|---|---|
| `test/virtual-path.test.ts` | 路径校验：合法 / `..` / 绝对路径开头 / 超长 / 空段 / 控制字符 / UTF-8 文件名合法 |

**Gateway 侧（vitest，独立 `apps/gateway/test/` 目录，按子模块镜像 src）**

| 模块 | 关键用例 |
|---|---|
| `test/auth/container-token.test.ts` | 1) 有效 JWT + token_version 匹配 → 解出 user_id 2) 过期 JWT 401 3) 篡改的 JWT 401 4) **token_version 不匹配 → 401 (撤销场景)** 5) 缺 Authorization header 401 |
| `test/api/cloud.test.ts` `/sas` | 1) entitlement active 时返回 directory SAS（mock UDK）2) entitlement disabled_at NOT NULL 时 → 403 3) DK 服务异常 → 500 |
| `test/api/cloud.test.ts` `/list` | 1) prefix=空 列根目录 2) prefix=`reports/` 列子目录 3) 跨用户前缀越界 → 400 4) **gateway 返回 decoded metadata UTF-8 string** |
| `test/api/cloud.test.ts` `/download` | 1) `dispose=inline`（默认）签出 SAS 不带 rscd，302 2) `dispose=attachment` 签出 SAS 带 RFC 5987 rscd 含中文文件名 3) blob 不存在 → 404 4) path 含 `..` → 400 |
| `test/api/entitlements.test.ts` | 1) **enable→disable→enable 回路：disabled_at 清空，行 active** 2) 首次 enable 触发 restart + token bump 3) disable 不删 blob 4) **token_version 在 enable/disable 时 +1** |
| `test/api/me-observed.test.ts` | 1) 上报正确写入 container_observed_state 2) JWT 缺失 401 3) status 接口反映 observed |
| `test/api/auth-refresh.test.ts` | 1) token 临近过期 → 续签返回新 token（90 天） 2) **超期但在 grace（7 天内）→ 仍然续签**  3) 超过 grace → 401 4) token_version 不匹配 → 401 |
| `test/lib/user-delegation-key-cache.test.ts` | 1) 首次拿 → 调 Azure 2) 缓存内 → 不调 3) 过期 → 刷新 |
| `test/lib/sas-builder.test.ts` | 1) **生成的 SAS 包含 sr=d sdd=1 sv>=2020-02-10 spr=https** 2) canonicalized resource 正确 3) racwl 权限映射 4) 不支持的 perm 抛错 |
| `test/lib/content-disposition.test.ts` | 1) ASCII 文件名 → `attachment; filename="..."` 2) 含中文 → `attachment; filename*=UTF-8''<encoded>` 3) 含空格 / 特殊字符正确转义 |
...
| 2. entitlement enable 流程 + observed 回环 | enable API → 验证 DB 写入 + provisioner mock 被调用 restart → entrypoint mock 调用 /api/me/observed-entitlements 上报 → status 反映 desired ⊆ observed |
| 3. 多租户隔离（directory SAS 真验证） | 两个 user 各自 publish 同名 virtual_path → 验证 list 时只看到自己的，user A 试图用自己 SAS PUT 到 user B 前缀 → **Azure 服务端拒 403**（验证 sr=d+sdd 真生效） |
...
| 7. **disable → enable 回路恢复 active** | 启用 → 停用（disabled_at NOT NULL）→ 再启用 → 验证 disabled_at 清空、entrypoint 重新软链、observed 反映 cloud |
| 8. **token_version 撤销** | 拿一个有效 token → gateway 端调用 enable / disable 都会 bump version → 旧 token 立即 401 → 续签后新 token 通过 |
| 9. **refresh-token grace 期** | 模拟 token exp 在 1 / 6 / 8 天前 → 6 天内续签成功，8 天拒（grace=7d） |
| 10. **中文文件名 attachment 下载** | publish "销售报告.pdf" → /download?dispose=attachment → 验证响应 Content-Disposition 含 RFC 5987 filename* 头 |
...
| Phase | 标题 | 交付物 | 验证标准 |
|---|---|---|---|
| **P0** | 基建准备 | **创建 HNS-enabled (ADLS Gen2) storage account** + container `laifu-cloud`；`GATEWAY_SECRET` env；user delegation key cache 骨架；`packages/shared/src/lib/virtual-path.ts` + contracts 扩展；sas-builder 骨架（验证 sr=d+sdd 签出来的 token 实际能限定到子目录） | 手动测：用骨架签一个限定 `<test-uuid>/` 的 SAS，验证跨前缀的 PUT 被 Azure 拒 403 |
| **P1** | Entitlement + Token + Observed 闭环 | `user_entitlements` + `container_observed_state` 表 + `users.token_version` 列 + RLS；`/api/entitlements/cloud/enable\|disable`（含 token bump）；`/api/me/entitlements` `/api/me/observed-entitlements` `/api/auth/refresh-token`；container-token middleware（含 version 校验）；`/api/status` 扩展返回 desired + observed + containerStatus；容器 entrypoint 软链逻辑 + JWT 注入 + 启动时续签 + 上报 observed | 单测 + 集成测试：a) enable→disable→enable 回路正确恢复 active；b) bump token_version 后旧 token 立刻 401；c) refresh-token 在 grace 期内成功，超期拒；d) 容器 restart 后 observed_entitlements 反映新软链状态 |
| **P2** | Cloud 数据面 gateway | `/api/cloud/sas`（sr=d + sdd directory SAS） `/list`（解码 metadata） `/download`（含 dispose + rscd RFC 5987）；content-disposition 编码 lib | 单测 + 集成测试（test 3 多租户隔离 + test 5 注入防御 + 中文文件名 attachment 下载） |
...
```
新增 — packages/shared (gateway 和 web 共享):
  packages/shared/src/lib/virtual-path.ts                  (~80 行)
  packages/shared/src/contracts.ts                         (+~80 行 扩展云盘相关 types)

新增 — gateway 源码:
  apps/gateway/src/api/cloud.ts                            (~280 行 含 sas/list/download/dispose+rscd)
  apps/gateway/src/api/entitlements.ts                     (~140 行 含 upsert + token_version bump)
  apps/gateway/src/api/me-observed.ts                      (~60 行 observed-entitlements 上报)
  apps/gateway/src/api/auth-refresh.ts                     (~80 行 refresh-token 端点 含 grace)
  apps/gateway/src/auth/container-token.ts                 (~70 行 含 token_version 校验)
  apps/gateway/src/lib/user-delegation-key-cache.ts        (~70 行)
  apps/gateway/src/lib/sas-builder.ts                      (~80 行 sr=d+sdd directory SAS 拼装)
  apps/gateway/src/lib/content-disposition.ts              (~50 行 RFC 5987 中文文件名编码)
  apps/gateway/src/db/entitlements-dao.ts                  (~100 行)
  apps/gateway/src/db/observed-state-dao.ts                (~60 行)

新增 — gateway 测试 (独立 test/ 目录):
  apps/gateway/test/api/cloud.test.ts                      (~240 行)
  apps/gateway/test/api/entitlements.test.ts               (~140 行 含 disable→enable 回路)
  apps/gateway/test/api/me-observed.test.ts                (~60 行)
  apps/gateway/test/api/auth-refresh.test.ts               (~80 行 含 grace 期边界)
  apps/gateway/test/auth/container-token.test.ts           (~100 行 含 token_version 回退用例)
  apps/gateway/test/lib/user-delegation-key-cache.test.ts  (~80 行)
  apps/gateway/test/lib/sas-builder.test.ts                (~80 行 验证 sr=d/sdd/sv canonicalized)
  apps/gateway/test/lib/content-disposition.test.ts        (~60 行)

新增 — packages/shared 测试:
  packages/shared/test/virtual-path.test.ts                (~60 行 路径校验)
...
| Azurite 对 **directory-scoped User Delegation SAS (sr=d+sdd)** 的支持 | Azurite 对 HNS / ADLS Gen2 directory SAS 历史支持不全 | P0 开始前在 Azurite 跑一次 sr=d sdd 签发 + 跨前缀拒绝的实际测试；若不支持：a) 集成测试 fallback 用真 Azure dev storage（成本极低）；b) 单元层用真签发 + mock 验证；prod 仍走 UDK |
| `~/.hermes/skills/` 跨容器版本 hermes 接口稳定性 | 上游变接口风险 | 在 skill.md 里 pin 一个 protocol version |
| **HNS-enabled storage account 的成本** | ADLS Gen2 比扁平 Blob 略贵（~10%）+ 不支持某些 Blob 特性 | 接受成本差；MVP 不用 lifecycle policy / blob versioning / immutability 等需要协调的特性 |
| **token_version bump 与容器重启不原子** | DB bump 成功但 ACA restart 失败时，旧 token 已失效，新 token 没下发 → 容器现有连接 401 | enable/disable 流程在 DB bump 前先 issue 新 token；restart 失败时回滚 token_version（DB transaction 内）。详见 §五 enable 实现 |
| **refresh-token grace 期定 7 天是否合理** | 短了容器睡久醒来无法续签；长了泄露 token 撤销不及时 | MVP 取 7 天；P1 验收时观察实际容器 sleep 时长分布，必要时调到 30 天 |
