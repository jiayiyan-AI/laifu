...
>
> ⚠️ **CAE 自带 Standard Load Balancer 隐藏成本**: 给 Container Apps Environment 配 VNet (我们必须配, 为了把 NFS account 锁在 Service Endpoint 里) 之后, Azure 会在一个隐藏的 `ME_*` 托管 RG 里自动创建一个 Standard LB + Standard Public IP, 暴露给所有 ACA Ingress 用。我们 RG 里看不到, Bicep 也声明不到, 但**账单照收**: 实测 SEA 区 dev 环境 **~$22 USD/月固定**(基础 ~$18 + Static IP ~$3.6 + 2 条规则 ~$0.12)。**不随 ACA 数量增长** — 100 个用户 ACA 共用同一个 LB, 仍只收一份。下面成本表已把这一项加进去。
...
| **Standard Load Balancer** (CAE 自动建, 隐藏在 `ME_*` RG) | LB 基础 + Public IP + 规则 | **~$22/月固定** | SEA 区实测: 基础 ~$18 + Static IP ~$3.6 + 2 条规则 (HTTP/HTTPS) ~$0.12。不论 ACA 数量, 1 个 CAE 收 1 份 |
| **Container Registry (ACR Basic)** | SKU 月租 | **$5/月固定** | Basic SKU 含 10 GiB 存储 + 2 个 webhook。Standard $20/月, Premium $50/月 |
| | Build 任务 | ~$0.0001/s | 推一次 Hermes 镜像 ~$1-3, 不推不收。频繁迭代时一周 1-2 次 |
| **Log Analytics** | 数据摄入 | $2.76/GB | 30 天保留期, 当前用量 <0.5 GB/月, 月费 <$1, 早期可忽略 |
...
xychart-beta
    ...
    y-axis "月成本 ($)" 0 --> 1100
    bar [57, 57, 83, 183, 1012]
```

| 规模 | App Service | Container Apps 计算 | NFS 共享 share | 云盘 Blob | Standard LB (CAE 自带) | ACR Basic | **月总成本** |
|------|------------|-------------------|----------------|-----------|---------------------|-----------|------------|
| **1 用户** (当前 dev) | $13 | $0（免费额度内） | $16 | ~$0 | $22 | $5 | **~$57** |
| **10 用户** | $13 | $0（免费额度内） | $16 | ~$1 | $22 | $5 | **~$57** |
| **50 用户** | $13 | ~$20 | $16 | ~$3 | $22 | $5 | **~$83** |
| **100 用户** | $13 | ~$120 | $16-32 (撞顶时升预配) | ~$6 | $22 | $5 | **~$183** |
| **500 用户** | $26（升 B2） | ~$900 | ~$80 (升到 500 GiB) | ~$26 | $22 | $5 | **~$1,012** |
...
>
> **CAE 自带 Standard LB ($22) + ACR Basic ($5) 是固定地板**: 不论 1 用户还是 500 用户都收。规模化后占比可忽略, 但早期它们和 App Service B1 + NFS share 一起把月底价垫到 ~$57, 这是 ACA + VNet + 私有镜像仓库模式无法绕过的硬成本 (除非整体换计算层, 见 7.6)。
>
> **实测对照** (2026-06): 当前 dev 环境 0 业务用户, Azure 账单 API 推算月度 ~$67.5/月 — 比上表 1 用户行的 $57 多出 ~$10, 来自这周频繁推镜像的 ACR Build 任务 ($11) + Blob 事务费 ($3), 都是临时项, 稳态会回到 $57 附近。
>
> 真正的规模成本主导项仍是 **Container Apps 计算费** — 500 人量级接近 $900, 这是正常的"按用 → 按计算量计费"结构。
...
> **核心结论**：早期 Azure 基础设施成本 ~$52/月 (App Service $13 + 固定 NFS share $16 + CAE 自带 LB $22 + Blob ~$1), 与用户数无关。规模化后 Container Apps 按秒计费成为主要支出项（500 人量级 ~$1,007/月），需要在产品定价时充分考虑这一项。共享 share + subPath 方案让存储部分基本恒定, 不再是早期"每用户 $16 线性增长"的成本噩梦; LB 是 CAE+VNet 模式的固定地板, 不随用户数变化。
...
| **CAE Standard LB (自带)** | **$22/月固定** (LB 基础 $18 + Static IP $3.6 + 规则 $0.12) | **$0** (Fly Machines 自带 anycast IP, 不另收) |
...
| CAE Standard LB | $22 | — | $22 | — | $22 | — | $22 | — |
| 带宽 | — | ~$0.5 | — | ~$2 | — | ~$4 | — | ~$20 |
| **合计** | **~$52** | **$19.3** | **~$74** | **$75.8** | **~$177-193** | **$145.5** | **~$1,054** | **$709.5** |

> 容器计算假设中度用户（日均 20 轮 × 60s）。Azure：1 vCPU + 2 GiB；Fly.io：performance-1x（1 dedicated vCPU + 2 GiB），算力对等。停机存储按 Docker image ~1.5GB rootfs 估算。Azure 持久化按 100 GiB NFS share ($16) + 200 GiB (100 人撞顶时升预配) + 500 GiB (500 人) 估算; Fly 端按每用户 1 GB 预分配。**Azure 多出的 $22/月 CAE Standard LB 是 ACA + VNet 模式的强制项**, Fly.io 自带 anycast IP 不另收。
...
|------|------------|-------------------|
| **早期成本（≤10人）** | ⚠️ ~$52 (NFS $16 + App Service $13 + CAE LB $22 三件套是地板) | ✅ ~$19, 从第一个用户起算 |
| **中期成本（50-100人）** | ⚠️ $74-193, 共享 NFS + 固定 LB 让增速极平 | ⚠️ $76-146, 与 Azure 互有胜负 |
| **规模成本（500人）** | ❌ ~$1,054 | ✅ ~$710 |
| **成本结构** | 共享 NFS 固定 + CAE LB 固定 + 计算贵 + 免费额度缓冲 | 计算便宜、存储按 GB 预分配 + 停机也收费 |
...
| **缩容到 0** | ⚠️ ACA 计算到 0, 但 CAE LB $22/月地板拿不掉 | ⚠️ 停机仍收 rootfs 存储费 |
