...
- ✅ PostgreSQL — dev 本地 docker PG, prod 走 Supabase Cloud (或 Azure DB for PG, 改 `DATABASE_URL` 即可)
- ✅ DashScope — dev/prod 用各自 key, 通过 `hermes-api-key` 注入容器 (一身二用)
...
# 起本地 PG (postgres:17-alpine, 端口 54422):
./scripts/dev-db.sh start

# 首次启动后建表 + 灌种子:
DATABASE_URL=postgres://postgres:postgres@localhost:54422/postgres pnpm --filter @lingxi/db db:push
DATABASE_URL=postgres://postgres:postgres@localhost:54422/postgres pnpm --filter @lingxi/db db:seed

cp docker/hermes/.env.example docker/hermes/.env
# 填 HERMES_API_KEY (DashScope key 或 Anthropic key, 按 hermes-config 选)
...
1. 走 6 步假进度 (`'正在生成数字助理实例'` → 100%)，每步往 PG `container_mapping` 写 `provisioning_step` + `progress_pct`，让前端 `/api/status` 能看到进度条
...
最后取 `properties.configuration.ingress.fqdn` 作为 `container_url` 写回 PG。每用户拿到自己独立的 `https://hermes-<userId>.xxx.azurecontainerapps.io`。
...
```ts
const mapping = cache.get(userId);              // 从 PG 查
...
| 场景 | 推荐 |
|---|---|
| 大多数本机开发 | 本地 docker PG (`./scripts/dev-db.sh start`, 端口 54422), 离线友好 |
| 想用手机/平板访问 dev 实例 | 让 dev gateway 直连云上 dev DB (浏览器/移动端都能连) |
| 多人协作要共享数据 | 共享一份云上 dev DB |
| 想模拟接近生产的网络延迟 | 云上 dev DB |

切换很容易: 换 `.env.local` 里 `DATABASE_URL` 一行, 重启 gateway。Schema 变更走 Drizzle: `pnpm --filter @lingxi/db db:push` (本地) / `db:generate --name xxx` + `db:migrate` (云上)。详见 [packages/db/README.md](../packages/db/README.md)。
...
│  SESSION_SECRET, GATEWAY_SECRET, DATABASE_URL,       │
...
- ❌ dev 只用 `db:push` 改云上库, 不写 migration 文件 (生产同步会乱; 用 `db:generate` + `db:migrate`)
