...
// 渠道入站附件缓存 TTL (天)。entrypoint sweep + /inbox 落盘后 best-effort sweep 共用。
...
// 入站附件（微信、飞书等）落在 laifu-inbox/ —— 我们独占的子目录，故意**不**复用 Hermes
// 自己的 cache/images（那里 vision 工具会丢下载图/生成图，共享会被 TTL sweep 误杀）。
// 各类附件分目录保存；sweep 只扫 laifu-inbox/ 子树，绝不碰 cache 其余部分。
