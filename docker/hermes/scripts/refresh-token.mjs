...
import { log, warn, readToken, writeToken, decodeJwtPayload, httpJson } from './lib.mjs';
...
export async function runRefreshToken() {
  ...
  let exp = 0;
  try {
    exp = decodeJwtPayload(token).exp ?? 0;
  } catch (e) {
    warn(`failed to decode token: ${e.message}`);
    return;
  }
  ...
  try {
    ...
    warn(`refresh-token request failed: ${e.message} — keeping old`);
  }
}
