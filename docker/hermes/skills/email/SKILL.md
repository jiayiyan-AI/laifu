...
任何子命令返回 `{"ok":false,...}` 或退出码非 0,都表示这次邮件操作**没成功**。此时:

- **如实回报**:把 `error` 的内容直接告诉用户,例如"邮件发送失败:<error>"。
- **就此打住**:不要重试同一命令、不要换参数反复试、不要为此去问无关的澄清(clarify)、**绝不能谎称"已发送/已完成"**。
- 按退出码给用户一句可操作提示:2=鉴权失败(token 问题,通常需重新登录或联系管理员);1=参数缺失(补全 `--to`/`--body` 等再试);3=网关拒绝(收件人不存在、邮件 id 不存在等)。
